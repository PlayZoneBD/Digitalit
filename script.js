// ============================================
// 🏆 DIGITAL IT INSTITUTE - LOGIC SCRIPT
// ============================================
...
function checkLoginStatus() {
    ...
    if (user && user.isLoggedIn) {
        ...
        navBtn.classList.replace('text-slate-900', 'text-white');
        navBtn.onclick = logout;

        document.getElementById('dashUserName').innerText = user.name || "Student";
        syncUserPlan(user);
    } else {
        setNotice("Welcome to Digital IT Institute! Enroll in our premium courses to unlock lifetime access to professional automation tools.");
    }
}
...
function updateUIBasedOnPlan(user) {
    ...
    if (isPremium) {
        setNotice("Dashboard Access Granted. Your tools are unlocked and ready to use.");
        dashPlan.innerText = `${plan} ✅`;
        dashPlan.className = "text-emerald-400 font-bold";
        ...
        // UNLOCK All Icons
        const unlockClass = "ph-fill ph-check-circle absolute top-5 right-5 text-emerald-400 text-xl transition z-10";
        ['ua', 'email', 'validator', 'cpa', 'proxy', 'address'].forEach(id => {
            const icon = document.getElementById(`lock_${id}`);
            if(icon) icon.className = unlockClass;
        });

    } else {
        setNotice("Your account is restricted. Please purchase an IT course package to unlock workspace tools.");
        ...
        // LOCK All Icons
        const lockClass = "ph-fill ph-lock-key absolute top-5 right-5 text-slate-500 group-hover:text-red-500 transition text-xl z-10";
        ['ua', 'email', 'validator', 'cpa', 'proxy', 'address'].forEach(id => {
            const icon = document.getElementById(`lock_${id}`);
...
function startCountdown(expiryTimestamp) {
    ...
    if (!expiryTimestamp) {
        timerDiv.innerHTML = `<div class="text-emerald-400 font-bold text-sm bg-emerald-500/10 px-4 py-2 rounded-lg border border-emerald-500/20">LIFETIME ACCESS</div>`;
        return;
    }
    ...
    function update() {
        ...
        timerDiv.innerHTML = `
            <div class="flex gap-2">
                <div class="bg-slate-900 border border-slate-700 rounded-lg p-2 min-w-[50px]"><span class="text-lg text-white font-mono font-bold block leading-none">${days}</span><span class="text-[9px] text-slate-500">DAYS</span></div>
                <div class="bg-slate-900 border border-slate-700 rounded-lg p-2 min-w-[50px]"><span class="text-lg text-white font-mono font-bold block leading-none">${hours}</span><span class="text-[9px] text-slate-500">HRS</span></div>
                <div class="bg-slate-900 border border-slate-700 rounded-lg p-2 min-w-[50px]"><span class="text-lg text-white font-mono font-bold block leading-none">${minutes}</span><span class="text-[9px] text-slate-500">MIN</span></div>
            </div>
        `;
    }
...
function handleHomeClick(planValue = 'Digital Marketing Foundation|2000 BDT') {
    const user = JSON.parse(localStorage.getItem('proToolsUser')); 
    if (!user || !user.isLoggedIn) { 
        openAuthModal(); 
    } else { 
...
function switchTab(tab) { 
    ...
    if (tab === 'login') { 
        ...
        tabLogin.className = "flex-1 py-4 text-sm font-bold text-emerald-400 border-b-2 border-emerald-500 bg-slate-800 transition"; 
        tabRegister.className = "flex-1 py-4 text-sm font-bold text-slate-400 hover:text-white transition relative"; 
    } else if (tab === 'register') { 
        registerForm.classList.remove('hidden'); 
        tabRegister.className = "flex-1 py-4 text-sm font-bold text-emerald-400 border-b-2 border-emerald-500 bg-slate-800 transition relative"; 
        tabLogin.className = "flex-1 py-4 text-sm font-bold text-slate-400 hover:text-white transition"; 
    }
}
...
function handleAuth(event, action) { 
    ...
    .then(result => { 
        ...
        if (result.result === 'success') { 
            msgDiv.className = "px-8 pb-6 text-center text-sm font-bold text-emerald-400 bg-slate-900 pt-4 border-t border-slate-800"; 
            ...
            msgDiv.className = "px-8 pb-6 text-center text-sm font-bold text-red-400 bg-slate-900 pt-4 border-t border-slate-800"; 
            msgDiv.innerText = result.message; 
        } 
    })
    .catch(err => { 
...
function openPaymentModal(planValue = 'Digital Marketing Foundation|2000 BDT') {
